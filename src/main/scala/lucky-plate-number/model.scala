...
object LuckyPlate {
  ...
  def main(args: Array[String]) = {
    ...
    println("กรุณาเลือกจุดประสงค์ที่ต้องการ (เลือกตัวเลขได้มากกว่า 1 ข้อ ใช้ , ในการขั้นตัวเลือก)")
  ...
  def searchGoodPlateNumber(plateNumbers: Set[PlateNumber], unluckyNumberByDay: String, luckEnhanceNumbers: Set[LuckyNumber]): Set[PlateNumber] = {
    ...
    val result = getLuckyNumbers(luckyPlateNumbers, luckyNumbers)
    println(s"\n\nsearchGoodPlateNumber: ${result.size}")
    result
  }
  ...
  def filterOutUnluckyNumbers(plateNumbers: Set[PlateNumber]): Set[PlateNumber] = {
    ...
    println(s"\n\nbadNumbers: ${badNumbers.size}")
    val result = plateNumbers.filter(plate =>
      badNumbers.forall(badNumber => !plate.plateNumber.contains(badNumber))
    )
    
    println(s"\n\nfilterOutUnluckyNumbers: ${result.size}")
    result
  }

  def filterOutPlateNumberStartAndEndWithOne(plateNumbers: Set[PlateNumber]): Set[PlateNumber] = {
    val result = plateNumbers.filterNot(plate => plate.plateNumber.length == 4 && plate.plateNumber.head == '1' && plate.plateNumber.last == '1')
    println(s"\n\nfilterOutPlateNumberStartAndEndWithOne: ${result.size}")
    result
  }

  def filterOutPlateNumberSumEqualsThirteen(plateNumbers: Set[PlateNumber]): Set[PlateNumber] = {
    val result = plateNumbers.filterNot(plate =>
      filterOutNumberSumEqualsThirteen(plate.plateNumber)
    )
    println(s"\n\nfilterOutPlateNumberSumEqualsThirteen: ${result.size}")
    result
  }

  def filterOutNumberSumEqualsThirteen(plateNumber: String) = {
  ...
  def filterOutUnluckyPlateNumberByDay(plateNumbers: Set[PlateNumber], unluckyNumberByDay: String): Set[PlateNumber] = {
    val result = plateNumbers.filterNot(plate => plate.plateNumber.contains(unluckyNumberByDay))
    println(s"filterOutUnluckyPlateNumberByDay: ${result.size}")
    result
  }
}
